...
import { computeCreditSummary } from '../finance/financeLogic.js';
...
export function professionalAdviceRoutes(app) {
...
- Always respond in ${responseLanguage}`;
        ...
        const j = core.json || {};
        if (!j?.ok)
            return res.status(core.status).json({ ok: false, error: String(j?.error || 'Advice failed'), details: j?.details });
