...
export function registerBrainRouter(app: any) {
  ...
  app.post('/v1/analyst-agent', async (req: Request, res: Response) => {
    const auth = verifyApp(req)
    if (!auth.ok) return res.status(401).json({ ok: false, error: auth.error || 'Unauthorized', app_id: auth.app_id })
    const { mod } = resolveModule(auth.app_id)
    const out = await mod.professionalAdvice({ app_id: auth.app_id, user_id: null }, readHeader(req, 'host'), req.body)
  ...
  app.post('/v1/subscription/status', async (req: Request, res: Response) => {
    const auth = verifyApp(req)
    if (!auth.ok) return res.status(401).json({ ok: false, error: auth.error || 'Unauthorized', app_id: auth.app_id })

    const stripeKey = safeTrim(process.env.STRIPE_SECRET_KEY || process.env.STRIPE_API_KEY || '')
    if (!stripeKey) return res.status(503).json({ ok: false, error: 'Stripe is not configured on Brain (STRIPE_SECRET_KEY).' })

    const user_id = safeTrim((req.body as any)?.user_id || '')
    ...
    const active = Array.isArray(subs?.json?.data) ? subs.json.data.find((x: any) => x?.status === 'active' || x?.status === 'trialing') : null
    if (!active) {
      return res.json({ ok: true, tier: 'free', premium_until: null, trial_until: null, source: 'brain_stripe' })
    }
  ...
  async function acceptSupportTicket(req: Request, res: Response) {
    const auth = verifyApp(req)
    if (!auth.ok) return res.status(401).json({ ok: false, error: auth.error || 'Unauthorized', app_id: auth.app_id })
