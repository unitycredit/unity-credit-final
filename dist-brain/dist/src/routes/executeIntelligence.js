...
export function executeIntelligenceRoutes(app) {
    app.post('/v1/execute-intelligence', async (req, res) => {
        ...
        const j = core.json || {};
        if (!j?.ok)
            return res.status(core.status).json({ ok: false, blocked: Boolean(j.blocked), error: String(j.error || 'Intelligence execution failed'), details: j.details || undefined });
