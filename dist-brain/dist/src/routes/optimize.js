...
export function optimizeRoutes(app) {
    app.post('/v1/brain/optimize', async (req, res) => {
...
`;
        ...
        const j = core.json || {};
        if (!j?.ok)
            return res.status(core.status).json({ ok: false, error: String(j?.error || 'Optimization failed'), details: j?.details });
        ...
        return res.json({ ok: true, verified: Boolean(j?.verified ?? true), verification: j?.verification || null, ...shaped });
    });
}
