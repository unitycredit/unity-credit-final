...
export function logicProcessRoutes(app: any) {
  app.post('/v1/logic/process', async (req: Request, res: Response) => {
    ...
    const topItems =
      Array.isArray(ctx?.items) && (ctx as any).items.length > 0
        ? (ctx as any).items
    ...
    const financeSnapshot = createFinanceSnapshot({
      bank: ctx?.bank
        ? {
            monthly_income: typeof (ctx as any).bank.monthly_income === 'number' ? (ctx as any).bank.monthly_income : null,
            monthly_expenses: typeof (ctx as any).bank.monthly_expenses === 'number' ? (ctx as any).bank.monthly_expenses : null,
            total_balance: typeof (ctx as any).bank.total_balance === 'number' ? (ctx as any).bank.total_balance : null,
            accounts_count: typeof (ctx as any).bank.accounts_count === 'number' ? (ctx as any).bank.accounts_count : null,
          }
        : null,
      budget_items: Array.isArray(ctx?.items) ? (ctx!.items! as any) : null,
    })
...
Response language: ${preferYiddish ? 'Yiddish (Heimishe style)' : 'English'}`
    ...
    if (!j?.ok) return res.status(core.status).json({ ok: false, error: String(j?.error || 'Intelligence failed'), degraded: core.status >= 502 })

    // Best-effort central storage
    try {
