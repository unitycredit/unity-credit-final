...
export function agentRoutes(app: any) {
  ...
  app.post('/v1/agents/execute', async (req: Request, res: Response) => {
    ...
    if (!agent) return res.status(404).json({ ok: false, error: 'Unknown agent_id' })
...
Response language: ${preferYiddish ? 'Yiddish (Heimishe style)' : 'English'}`
    ...
    if (!j?.ok) return res.status(core.status).json({ ok: false, error: String(j?.error || 'Execution failed'), details: j?.details })
